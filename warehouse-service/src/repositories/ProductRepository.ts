...
export default class ProductRepository {
    ...
    static getInstance(ProductModel: mongoose.Model<Product>, PictureModel: mongoose.Model<Picture>) {
        return this._instance || (this._instance = new this(ProductModel, PictureModel));
    }
    ...
    postPicture = async (picture: Picture): Promise<any> => this.PictureModel.insertMany([picture]);

    getPicture = async (filter: {_id: string}): Promise<any> => this.PictureModel.find(filter);

    deleteProduct = async (filter: {[key: string]: string}): Promise<DeletionResult> => {
        const result = await this.ProductModel.deleteOne(filter);
        console.log('Deleted ', result);
        return result;
    };

    updateProduct = async (filter: {[key: string]: string}, product: Product): Promise<DeletionResult> => {
