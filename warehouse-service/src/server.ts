...
import WarehouseRepository from "../../warehouse-service-cleaner/src/repositories/WarehouseRepository";
...
import {
    CannotCreateAdminException, CannotCreateConsumerException,
    CannotCreateProducerException, CannotCreateTopicException,
...
const run = async () => {
    ...
    const warehouseRepository = WarehouseRepository.getInstance(WarehouseModel, ProductModel);
    const productRepository = ProductRepository.getInstance(ProductModel, PictureModel);

    const productService = ProductService.getInstance(productRepository);
    const warehouseService = WarehouseService.getInstance(warehouseRepository, productService, producerProxy);

    const warehouseController = WarehouseController.getInstance(warehouseService, productService);
    const productController = ProductController.getInstance(productService, warehouseService);
    ...
    const app = await getApp(rootPath, productController, warehouseController, producerProxy);
